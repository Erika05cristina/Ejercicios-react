...
const TOKEN =
  ...
  test.describe("When logged in", () => {
    ...
    test('blogs are sorted by likes', async ({ page }) => {
      ...
      await page.click('button:has-text("Create new blog")');
      await page.fill('input[name="title"]', 'Blog A');
      await page.fill('input[name="author"]', 'erika');
      await page.fill('input[name="url"]', 'https://blogA.com');
      await page.fill('input[name="likes"]', '5');
      await page.click('button[type="submit"]');
  
      await page.click('button:has-text("Create new blog")');
      await page.fill('input[name="title"]', 'Blog B');
      await page.fill('input[name="author"]', 'erika');
      await page.fill('input[name="url"]', 'https://blogB.com');
      await page.fill('input[name="likes"]', '35');
      await page.click('button[type="submit"]');
  
      await page.waitForSelector('div.blog-summary');
  
  
      const blogsLikes = await page.locator('div.blog-summary')
        .allTextContents();
  
      const likes = blogsLikes.map(blog => {
